...
| 项目 | 内容 |
|---|---|
| 文档版本 | v0.15（注入链同步化：TextInjecting 改 @MainActor 同步，消除跨执行器 hop 卡死） |
| 日期 | 2026-08-20 |
| 目标平台 | macOS |
| 技术路线 | Swift + SwiftUI/AppKit 原生 |
| 状态 | MVP 开发中（Phase 0–8 已实现并验证，见 `PLAN.md`） |
...
### v0.11 → v0.12 变更记录（2026-08-19）

- Phase 8 结果注入落地（FR-F1 / FR-F4 / FR-F5）：§4.2.6 更新为 as-built 实况——剪贴板快照 → 写入 → `CGEventPost` 模拟 Cmd+V → 约 300ms 后按 `changeCount` 竞争保护恢复原剪贴板；无辅助功能权限 / 无有效注入目标（pid 0 或空 bundleID）降级为仅剪贴板（文本留存供手动粘贴，不恢复）；CLI 目标（terminal）默认折叠换行为空格（`inject.collapseNewlines`，默认 true，可关）；FR-F4 自动发送默认关（`inject.autoSend`），粘贴后约 150ms 模拟 Return。§4.2.6 移除 v0.6「Phase 8 落地前 interim」注记。

### v0.12 → v0.13 变更记录（2026-08-19）

- 润色防脑补二次加固（真机反馈：Terminal 下口述"可以了，提交吧"被润色成"提高 Terminal 窗口的显示效果"）：§9.1 润色 System Prompt 6 条→7 条——输出必须与口述表达同一件事、口述里没有的对象/动作/步骤一律不得出现、短促对话/流程用语按字面通顺化不扩展；§4.2.4 同步——terminal 目标省略窗口标题（进程名/TMPDIR/尺寸等噪声大、指代价值低，只保留 App 名）。

### v0.13 → v0.14 变更记录（2026-08-19）

- 润色防脑补三次加固（真机反馈：口述"两个都做吧"被润色成"执行两个命令"，引入原文没有的"命令"一词）：§9.1 润色 System Prompt 第 2 条新增——禁止把口述里的泛化动词替换成更具体的动词（如把"做/弄"改成"执行/生成/优化"）；第 5 条扩展——短促对话/流程/应允用语（"两个都做吧""都做吧"等）绝不替换动词、绝不补全"这个/那个/两个/它"等指代所指的对象、绝不引入原文没有的名词（如"命令/任务/操作"），宁可原样保留也不自行补全。

### v0.14 → v0.15 变更记录（2026-08-20）

- Phase 8 实现偏差回写（真机 bug：注入偶发卡「注入中」，两轮跨执行器 hop 修复后仍未根治）：§9.1 `TextInjecting` 协议从 `async -> InjectionOutcome` 改为 **`@MainActor` 同步方法**，注入链（剪贴板/CGEvent/延迟恢复）全部主线程同步执行，延迟恢复走 `DispatchQueue.main.asyncAfter` 调度——彻底消除 @MainActor 上下文 await nonisolated async 的跨执行器 hop（偶发丢 continuation，非崩溃非阻塞）。细节见 `docs/implementation-notes.md`「注入偶发卡在注入中」。
...
- **降级与竞争**：
  ...
  - （v0.12 落地）Phase 8 已实现上述 P0 流程：完整剪贴板+模拟 Cmd+V（含快照/恢复/changeCount 竞争保护）、CLI 换行折叠、FR-F4 自动发送（默认关）；无辅助功能权限 / 目标无效（pid 0 或空 bundleID）降级仅剪贴板且不恢复原剪贴板（文本留存供手动粘贴）。AX 写入（axWritten）与 CLI stdin 直通为 P1 未做。
...
4. **商业形态：开源 + Pro 订阅，还是闭源买断？** ✅ 已决策（2026-08-19 定形）
   决定：**采用 MIT 许可证开源**（替代「暂缓定形」）。代码按可开源组织；官网走 GitHub Pages，分发走 GitHub Releases；MCP Server 随开源公开。商业化增强（Pro）不排除，但不阻塞开源。
5. **产品命名与品牌方向。** ✅ 已决策
...
protocol TextInjecting: Sendable {
    ...
    /// 同步方法：注入链全程主线程（NSPasteboard/CGEvent 主线程访问 + 延迟调度），避免跨执行器 hop
...
`Sendable` 约束说明（Swift 6 严格并发）：`TranscriptionEngine` / `PromptRefining` 的实现会被 @MainActor 的 VoicePipeline 跨隔离域异步调用，协议必须要求 `Sendable`；`TextInjecting` 自 v0.15 起为 `@MainActor` 同步协议（注入链零 hop）；`AudioCapturing`（主线程同步调用）与 `ContextCollecting` 不作此要求。

润色 System Prompt（v0.12，2026-08-19 防脑补二次加固）：
