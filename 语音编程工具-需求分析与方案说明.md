...
| 项目 | 内容 |
|---|---|
| 文档版本 | v0.6（Phase 5 实现偏差回写：模型下载策略、语音识别权限、注入占位 interim） |
| 日期 | 2026-08-18 |
| 目标平台 | macOS |
...
- **模型下载策略**（§8-2 已决策；v0.6 按 Phase 5 实现回写）：启动即后台自动下载 small（不打断首启流程），设置页可见下载进度与失败重试；采用 WhisperKit 内置下载（底层即 URLSession 后台 session，自带断点续传）；落盘校验分层：下载管理器校验模型目录存在性，引擎激活时 `loadModels()` 成功兜底（不做逐文件校验）；模型落盘 `~/Library/Application Support/Voxmit/Models`；下载完成前自动以 Speech 框架兜底；Intel 机型跳过下载、直接引导云端 ASR；
