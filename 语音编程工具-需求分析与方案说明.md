...
```
VoicePrompt.xcodeproj
VoicePrompt/
├── VoicePromptApp.swift        # @main；MenuBarExtra + Settings 场景
├── Pipeline/
...
VoicePromptTests/               # 协议 mock + 核心逻辑单测（§9.3）
```
...
> 第 1–4 项已于 2026-08-17 经维护者决策确认（即原 v0.2 建议方案全部采纳）；第 5 项维持暂名。
...
5. **产品命名与品牌方向。** ⏳ 未决（不阻塞）
   决定：维持暂名 VoicePrompt，V1.1 结束前定名（需查重 App Store / GitHub / 商标）。
