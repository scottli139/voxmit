...
| LLM 润色（云端，非流式一次性返回） | ≤ 800 ms | 总超时 4.3s（v0.9 放宽，见 §4.2.4），超时回退原文 |
...
- **关键实现**：
  ...
  - 超时：总预算 4.3s（首次尝试 2.0s + 退避 300ms + 重试 2.0s；2026-08-19 由 3s/1.2+0.3+1.5 放宽——Moonshot 响应波动 + 冷连接握手致 3s 下超时回退率过高，"润色成功但略慢"优于"快但未润色"）；超时/失败/未配置 Key 均回退原文，返回值带"是否润色"标记供 HUD 角标展示；
