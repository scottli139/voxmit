...
- **剪贴板快照/恢复**：`pasteboardItems`（nullable，Swift 侧 `[NSPasteboardItem]?`，SDK 头文件确认）取快照后必须**用取出的数据重建全新 NSPasteboardItem 再 `writeObjects` 恢复**——系统 vended 的 item 永久关联原 pasteboard，直接回写必抛 `NSInvalidArgumentException`（"already associated with another pasteboard"，Swift 无法 catch → 主线程 abort）。`clearContents` 不会使已保存的 NSPasteboardItem 失效（对象独立于 pasteboard）；但 capture 侧仍 eager 固化各 type 的 Data，防惰性数据（promised data）在 300ms 后恢复时因来源 App 退出而失效。
- **剪贴板恢复写回崩溃（2026-08-20 真机 SIGABRT，反复 6 次复现）**：即上述"already associated"异常——崩溃时点为粘贴成功后约 300ms（延迟恢复闭包），表现为"用着用着自己退出"；空/非空剪贴板都会触发（实测 `writeObjects([])` 返回 true 不抛，空数组不是元凶）。修法：capture 重建全新 item（见上条），restore 对空快照直接放弃恢复。教训：桥接 AppKit 抛 NSException 的 API（`writeObjects:` 等），"对象可以回写"这类行为假设必须真机验证，不能凭"对象独立于 pasteboard"类推。
